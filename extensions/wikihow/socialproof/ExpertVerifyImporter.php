...
class ExpertVerifyImporter {
	...
	public function getSpreadsheet() {
		...
		$sheetData = json_decode( $sheetData );
		$sheetData = $sheetData->{'feed'}->{'entry'};
		$verifierResult = array_merge_recursive( $result, $this->parseGoogleVerifierResponse( $sheetData ) );
		$result = array_merge_recursive( $result, $verifierResult );
	...
	private function parseGoogleResponse( $data ) {
		...
		foreach( $data as $row ) {
			$worksheetName = "";
			$worksheetId = $row->{'id'}->{'$t'};
			foreach ( self::getWorksheetIds() as $wsId => $wsName ) {
				if ( strstr( $worksheetId, $wsId ) !== FALSE ) {
	...
	public function createExpertDoc( $service, $article, $name, $context, $folderId=self::EXPERT_FEEDBACK_FOLDER_ID ) {
