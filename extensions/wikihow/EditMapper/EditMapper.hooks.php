...
class EditMapperHooks {
	...
	public static function onPageContentSave(WikiPage $article, User &$user, Content $content,
		...
		$isNewArticle = $flags & EDIT_NEW;
		list($mapper, $destUser) = static::getActiveMapper($isNewArticle, $user);
		if ($mapper && $destUser) {
	 ...
	 */
	public static function getActiveMapper(bool $isNewArticle, User $user): array {
		$mappers = static::getAllEditMappers();
		foreach ($mappers as $mapper) {
			if ($mapper->shouldMapEdit($isNewArticle, $user)) {
				$destUser = $mapper->getDestUser($isNewArticle);
				if ($destUser) {
