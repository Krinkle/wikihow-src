	...
	*/
	...
	static function getLinksTo($fromLang, $fromPageId, $getTitles = true, $indirectLinks = false, $timeOrder = false) {
		...
		if ($fromLang != 'en' && $indirectLinks) {
			$sql = "select tl.tl_to_lang as tl_from_lang, tl.tl_to_aid as tl_from_aid, tl2.tl_to_lang, tl2.tl_to_aid from " . WH_DATABASE_NAME_EN . ".translation_link tl join " . WH_DATABASE_NAME_EN . ".translation_link tl2 on tl.tl_from_aid=tl2.tl_from_aid and tl.tl_from_lang='en' and tl2.tl_from_lang='en' and tl2.tl_to_lang <> tl.tl_to_lang where tl.tl_to_lang=" . $dbr->addQuotes($fromLang) . " and tl.tl_to_aid=" . $dbr->addQuotes($fromPageId);
			if ($timeOrder) {
		...
		foreach ($tls as $tl) {
			$dbName = Misc::getLangDB($tl->toLang);
			if ( !$dbName ) {
				continue;
			}
			$pageId = wfGetDB(DB_SLAVE)->selectField("{$dbName}.index_info", 'ii_page',
				[ 'ii_page' => $tl->toAID, 'ii_policy' => $indexablePolicies ]
			);
	...
	public static function beforePageDisplay(OutputPage &$out, Skin &$skin) {
		...
		$lls = array();
		foreach ($tls as $tl) {
			if ( !wfRunHooks( 'TranslationLinkAddLanguageLink', array( $tl ) ) ) {
				continue;
			}
			if (!$tl->isIndexable) {
				continue;
