...
class LSearch extends SpecialPage {
	...
	*/
	...
	private function externalSearchResultsYahoo($q, $start, $limit = 30, $gm_type = self::SEARCH_OTHER) {
		global $wgMemc, $wgBogusQueries, $wgCensoredWords, $IP;
...
//				echo $rsp;exit;
				...
				$this->getOutput()->addHTML(wfMessage('lsearch_query_error', $reqUrl)->plain());
				curl_close($ch);
				return null;
			} else {
				//echo $rsp;exit;
	...
	private function externalSearchResultsBing($q, $start, $limit = 30, $searchType = self::SEARCH_OTHER) {
		global $wgMemc, $wgBogusQueries, $wgCensoredWords;

		$key = wfMemcKey("BingSearchAPI", str_replace(" ", "-", $q), $start, $limit);
		...
		if (!is_array($contents)) {
			...
			if ($searchType == self::SEARCH_LOGGED_IN || $searchType == self::SEARCH_LOGGED_OUT) {
				$responseFilter = urlencode("Webpages,SpellSuggestions");
			} else {
				$responseFilter = "Webpages";
			}

			$queryUrl =  "https://api.cognitive.microsoft.com/bing/v5.0/search?responseFilter=$responseFilter"
			...
			curl_setopt($ch, CURLOPT_HTTPHEADER, ["Ocp-Apim-Subscription-Key: "
				. WH_AZURE_COGNITIVE_SERVICES_BING_API_SUBSCRIPTION_KEY]);
		...
		$this->mResults['results'] = $this->normalizeBingResults($contents['value']);

		$this->mResults['totalresults'] = $contents['totalEstimatedMatches'];
	...
	private function normalizeBingResults($results) {
		foreach ($results as $i => $r) {
			...
			$r['url'] = preg_replace(["@\x{E000}@u","@\x{E001}@u"], ["",""], $r['displayUrl']);
			if (!preg_match('@^https?://@', $r['url'])) {
				$r['url'] = 'https://' . $r['url'];
			}
			$results[$i] = $r;
		}
	...
	private function supplementResults($titles) {
		...
		$cachekey = wfMemcKey('supp-improved2', $this->mStart, $this->mLast, $enc_q);
	...
	/* We stopped using this method in June 2017 - Alberto

	// If we are not in English and not using lucene, fallback to the results from
	// the /Special:Search page and display those
	...
	*/
	...
	private function regularSearch() {
		...
		$enc_q = htmlspecialchars($this->mQ);
		$this->getOutput()->setHTMLTitle(wfMsg('lsearch_title_q', $enc_q));
	...
	private function displaySearchResults( $results, $resultsPerPage, $enc_q, $suggestionLink, $searchId ) {
		...
		$vars = array(
			'q' => $q,
			'enc_q' => $enc_q,
			'ads' => wikihowAds::getSearchAds($sk),
			'sk' => $sk,
			'me' => $me,
