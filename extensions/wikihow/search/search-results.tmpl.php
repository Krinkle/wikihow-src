		...
		<? if ($result['has_supplement']): ?>
			...
			<? if (!empty($result['first_editor'])): ?>
				...
				<? if (!empty($result['last_editor']) && $result['num_editors'] > 1): ?>
					<div>
						<?= wfMsg( 'lsearch_last_updated', wfTimeAgo(wfTimestamp(TS_UNIX, $result['timestamp']), true), Linker::link(Title::makeTitle(NS_USER, $result['last_editor']), $result['last_editor']) ) ?>
					</div>
				<? endif; ?>
			<? endif; ?>
