...
class CategoryData {
	...
	public function getAllData($sortKey = '', $limit = 48, $withFeatured = true) {
		return array(
			...
			'cat_articles' => $this->getArticles($sortKey, $limit, $withFeatured),
			'subcategories' => $this->getSubcategories()
		);
	}

	protected function getArticles($sortKey = '', $limit = 48, $withFeatured = true) {
		$rows = array();
	...
	protected function getArticlesFromDB($sortKey = '', $limit = 48, $featured) {
		$dbr = wfGetDB(DB_SLAVE);
		if ( RobotPolicy::isIndexable($this->t) ) {
