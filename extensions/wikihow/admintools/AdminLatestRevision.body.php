		...
		if(count($urls) > 0) {
			...
			if ($option == "chocothor") {
				...
				foreach ($userPages as $userPage) {
					$chocoUser = str_replace('http://www.wikihow.com/User:', '', $userPage);
				...
				$username = str_replace('http://www.wikihow.com/User:', '', $username);
			...
			foreach ($urls as $url) {
				...
				if ($title && $title->getArticleID() > 0) {
					...
					if ($option == "chocothor") {
						...
						$res = $dbr->select('revision', array('rev_id'), array('rev_user' => $user->getId(), 'rev_page' => $title->getArticleID()), __METHOD__, array('ORDER BY' => 'rev_timestamp DESC', 'LIMIT' => 1));
						$row = $dbr->fetchObject($res);
						if ($row) {
							$revisionUrl = $title->getCanonicalURL("oldid={$row->rev_id}");
						}
