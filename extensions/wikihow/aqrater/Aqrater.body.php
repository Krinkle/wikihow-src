...
class Aqrater extends UnlistedSpecialPage {
	...
	function processBatchRequest($posted_values) {
		...
		elseif(isset($posted_values['lastbatch'])) {
			...
			$row = $dbr->selectRow('aqrater.aqjobs',array('aq_id'),array('aq_status=2'),__METHOD__,array('ORDER BY' => 'aq_id DESC'));
			if($row) {
				$batchid=$row->aq_id;
				self::$errors[]=$batchid;
			}
	...
	private function getResults($batchid,$allData) {
		...
		// get data
		$res = $dbr->select('aqrater.pageratings',$colms,$conds,__METHOD__,array());
	...
	function processUpload($uploadfile,$userName) {
		...
		if (!move_uploaded_file($uploadfile,$fileSaveName)) {
			...
			$dbw = wfGetDB(DB_MASTER);
			$dbw->insert('aqrater.aqjobs', array('aq_status'=>'0','aq_filename'=>$fileSaveName,'aq_lasttimestamp'=>date("YmdHis"),'aq_newjobs'=>'1','aq_jobname'=> self::$jobName,'aq_jobmessage'=>$jobMessage,'aq_user'=>$userName), __METHOD__,array());
		}
